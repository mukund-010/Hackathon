...
// Connect to MongoDB (only for user preferences)
...
function getAvailableIngredients(region, season) {
    try {
        // Filter ingredients directly from memory instead of database
        const ingredients = indianFoodData.indianFoods.filter(food =>
            (food.availability.regions.includes(region) || food.availability.regions.includes("All India")) &&
            (food.availability.season === season || food.availability.season === "Available year-round")
        );
        return ingredients.map(food => food.name.toLowerCase());
    } catch (error) {
...
async function recommendRecipes(userData) {
    const {
        name,
        region,
        season = "Available year-round",
    ...
    try {
        // Get all recipes directly from the recipeData object
        ...
        return {
            ...
            recommendations: topRecommendations.map(recipe => ({
                name: recipe.name,
                matchScore: (recipe.score / 15 * 100).toFixed(0) + "%", // Convert to percentage... ingredients: recipe.ingredients,
...
// API Endpoint to Create User Preferences
...
app.post('/recommendations', async (req, res) => {
    ...
    try {
        const recommendations = await recommendRecipes(userData);
